...
视图控制器充当其管理的视图和应用程序数据之间的中介。当你继承 UIViewController 时，你可以添加任何需要在你的子类中管理数据的变量。视图控制器引用了你的数据和用于呈现该数据的视图。在两者之间来回移动数据是你的责任。![](/assets/A view controller mediates between data objects and views.png)你应始终在视图控制器和数据对象内保持清晰的职责分离。
...
内容视图控制器拥有他们的所有视图，并对这些视图中的数据负责。

容器视图控制器不拥有它的所有视图;其一些视图由其他视图控制器管理。
