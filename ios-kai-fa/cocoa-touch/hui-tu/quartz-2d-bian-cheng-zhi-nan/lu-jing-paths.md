...
您可以通过调用函数`CGContextSetBlendMode`来设置混合模式以获得各种效果，并传递适当的混合模式常量。请记住，混合模式是图形状态的一部分。如果在更改混合模式之前使用函数`CGContextSaveGState`，则调用函数`CGContextRestoreGState`会将混合模式重置为normal。

图3-13 在前景中绘制的矩形

![](/assets/The rectangles painted in the foreground.png)

图3-14 在背景中绘制的矩形

![](/assets/The rectangles painted in the background.png)

正常混合模式（`kCGBlendModeNormal`）：

![](/assets/Rectangles painted using normal blend mode.png)

乘法混合模式（`kCGBlendModeMultiply`）：

![](/assets/Rectangles painted using multiply blend mode.png)

屏幕混合模式（`kCGBlendModeScreen`）：

![](/assets/Rectangles painted using screen blend mode.png)

重叠混合模式（`kCGBlendModeOverlay`）：

![](/assets/Rectangles painted using overlay blend mode.png)
